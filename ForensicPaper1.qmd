...
You can review the abstract for this paper [here](https://journals.sagepub.com/doi/10.1177/0363546506296042). In brief, this was an attempted RCT comparing two ways of reconstructing the anterior cruciate ligament in patients participating in high levels of competitive sport at the time of injury. The target paper is the 10-year follow up of the comparison between groups, with previous reports at 2, 5 and 7 year follow ups.
...
Then I pulled out the last couple of years of citations.

```{r}
CitationRecent <- Citations |> dplyr::select(   year,   DO ) |> dplyr::filter(   year > 2022 )
```

I then took the DOIs and passed them to my reference management software [PaperPile](https://paperpile.com/about/) (PaperPile LLC, USA) and imported them to a folder. The software was able to update the metadata and retrieve fulltext files for those that were open access.
...
No prior trial registration in clinicaltrials.gov. Manual search performed 17-Feb-2025.
...
| Trial design                   | Absent            |                                                         |
+--------------------------------+-------------------+---------------------------------------------------------+
| Methods                        | Present           | Eliibility criteria for participants - but not settings |
|                                |                   |                                                         |
...
One systematic review [@janssen2017] used the Cochrane library checklist for RCTs, with two reviewers conducting independent ratings. The authors classified this trial as a *clinical controlled trial* (no formal randomisation). The overall rating from the combined checklist score was *Questionable (*score was between 30% and 50%). A more recent review [@sollberger2022] attempted to assess long term results after ACLR with PT vs HS autograft with a min follow up of 10 years, but did not include this paper in their analysis. Another review [@migliorini2020] included it in their quality assessment, but did not break down their RoB ratings by individual paper.
...
The intervention is reconstruction of the anterior cruciate ligament using autograft (material retrieved from the patient) during the operation and it is described the most in [@pinczewski2002]. There are numerous elements to the surgery that can vary;
...
-   Tunnel placement (tibia and femur)

-   Graft fixation (technique and hardware)

-   Closure

An important element of surgical outcomes in this context is the short-term rehabilitation approach. In the context of 10-year followup, the influence of rehabilitation might be relatively indirect, or certainly weakened compared to earlier follow ups.
...
We'll turn our attention to the time to event analysis of graft rupture.
...
The 10 year survival was 86% (95%CI 79 - 94) for the HT group and 92% (95%CI 86 - 98) for the PT group. This produces a hazard ratio of {round(92/86, 2)}.
...
calc_surv_prob <- function(data, time_point) {
  fit <- survival::survfit(
    survival::Surv(eventtime, status) ~ trt, 
    data = data
  )
  summary(fit, times = time_point)$surv
}

calc_ci <- function(data, time_point) {
  fit <- survival::survfit(
    survival::Surv(eventtime, status) ~ trt, 
    data = data
  )
  summ <- summary(fit, times = time_point)
  list(
    lower = summ$lower,
    upper = summ$upper
  )
...
calc_hr <- function(data) {
  ...
  exp(stats::coef(cox_fit))
}

# Set up simulation parameters
n <- 180
covs <- base::data.frame(
  id = base::seq(1, n),
  trt = base::rep(c(0, 1), each = n/2)  # 0 = PT group (reference), 1 = HT group
)

# Calculate required lambda for PT group (target survival = 0.92 at 10 years)
# Using the Weibull survival function: S(t) = exp(-λt^γ)
# For PT group: 0.92 = exp(-λ * 10^γ)
gamma_val <- 1.1  # Shape parameter
# Solving for lambda: λ = -ln(0.92)/(10^γ)
lambda_baseline <- -log(0.92)/(10^gamma_val)
lambda_ht <- lambda_baseline * 1.07

# Adjusted parameters
params <- base::list(
  lambdas = lambda_baseline,
  gammas = gamma_val,
  # For HT group target of 0.86, we need HR = 1.07
  betas = c(trt = log(1.07)),  # HT group has 1.07 times the hazard
  x = covs,
  maxt = 10
)

# Run simulation
sim_data <- base::do.call(simsurv::simsurv, params)
sim_data <- base::merge(sim_data, covs)

# Calculate metrics
surv_probs <- calc_surv_prob(sim_data, 10)
ci_bounds <- calc_ci(sim_data, 10)
hr <- calc_hr(sim_data)

# Print results
base::cat("PT Group (trt=0) 10-year survival:", base::round(surv_probs[1] * 100, 1), "%\n")
base::cat("PT Group 95% CI:", base::round(ci_bounds$lower[1] * 100, 1), "-", 
    base::round(ci_bounds$upper[1] * 100, 1), "%\n")
base::cat("HT Group (trt=1) 10-year survival:", base::round(surv_probs[2] * 100, 1), "%\n")
base::cat("HT Group 95% CI:", base::round(ci_bounds$lower[2] * 100, 1), "-", 
    base::round(ci_bounds$upper[2] * 100, 1), "%\n")
base::cat("Hazard Ratio:", base::round(hr, 2), "\n")
...
survival_plot <- ggsurvfit::ggsurvfit(survfit_object) +
  ...
  ggplot2::scale_color_manual(
    values = c("#2E9FDF", "#E7B800"),
    labels = c("PT Group", "HT Group"),
    name = "Treatment"
  ) +
  ggplot2::scale_fill_manual(
    values = c("#2E9FDF33", "#E7B80033"),
    labels = c("PT Group", "HT Group"),
    name = "Treatment"
  ) +
  ggplot2::labs(
    title = "Simulated Survival Curves by Treatment Group",
    x = "Time (years)",
...
ggplot() +
  ...
  labs(x = "Time (years)", 
       y = "Survival Probability", 
       title = "Simulated Survival Curves",
       color = "Group",
       fill = "Group") +
  theme_minimal() +
...
# Calculate 10-year survival probabilities with confidence intervals
summary(Graftfit, times = 10)

# Calculate observed hazard ratio
cox_model <- coxph(Surv(time, status) ~ group, data = dt)
summary(cox_model)
...
There is no mention of blinding in any of the articles related to this trial. Even if randomisation didn't occur, blinding of everyone involved in the trial (as much as practicable), but particularly those responsible for measurement and analysis is crucial to mitigate potential biases. Especially when industry funding is in the mix.
...
No subgroup analyses or adjusted analyses were performed. <!--# Double check this -->
...
Next, let's see if we can replicate a selection of the p-values. Let's simulate the groups. For simplicity, I've selected the *Ideal Outcome* binary outcome. This is a composite outcome variable derived from the IKDC clinical rating and radiographic osteoarthritis grading.
...
Mean p-value: {round(summary_stats$mean_pvalue, 4)}
95% CI for p-value: {round(summary_stats$p_value_95ci$interval[1], 4)} - {round(summary_stats$p_value_95ci$interval[2], 4)}
Mean effect size (difference in proportions):{round(summary_stats$mean_effect, 4)}
95% CI for effect size: {round(summary_stats$effect_95ci$interval[1], 4)} - {round(summary_stats$effect_95ci$interval[2], 4)}
Percentage of significant results: {round(summary_stats$sig_results, 1)}%

```

We'll check the percentages next
...
This is a bit of a mystery. Without access to a protocol prepared prior to the first follow up report, its difficult to know what has been omited.
...
This is described reasonably well in the text.
...
The question that arises from the present review is the lack of equipoise in the intervention groups and whether other centres with lower volume surgeons and different perioperative care expectations may have achieved different responses. Importantly, whether this trial still fits within a contemporary practice in the same it might have at the turn of the century remains a question.
...
Insert a balanced appraisal here
