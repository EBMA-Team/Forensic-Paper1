...
So I wrote a function (with the help of an LLM) to work through the surnames and bounce them against the RW database. Rest assured, I have used the underlying commands before, this was just a quicker way to put them together. We can argue about validation of this step until the cows come home, but for now I'm fairly satisfied that the individual authors do not have any retractions.
...
Let's see if we can replicate the p-values. First let's simulate the groups

```{r}
# Generate datatable with 2 groups of 90
StudySim <- simstudy::genData(149)
...
#Insert Outcome variables

# # Define a binary variable with different probabilities for each group
StudySimDef <- defDataAdd(
   varname = "IdealOutcome",
   formula = "ifelse(GraftGrp == 1, 0.47, 0.69)",# Prob 0.47 for group 1, 0.69 for group 2
dist = "binary"
   )

# Add the new variable to the dataset
 StudySim1 <- addColumns(def, StudySim1)


```


Let's switch the Group Assignment to a factor and run a Mann-Whitney U test on the outcome

```{r}

StudySim2 <- StudySim1 |> dplyr::mutate(
  GraftGrp = forcats::as_factor(
    GraftGrp
  ),
  IdealOutcome2 = forcats::as_factor(
    IdealOutcome
)
)
...
```
The larger p-value in the paper is likely because the sample sizes were smaller, indicating that the number of cases with IKDC available was smaller than the reported followup rate.

Let's rerun with the outcome as a categorical and apply an exact test instead.

```{r}

CountTable <- table(StudySim2$GraftGrp, StudySim2$IdealOutcome2)
