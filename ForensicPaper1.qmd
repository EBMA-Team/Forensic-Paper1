...
dt$Graft <- c(rep(0, 58), rep(1, 53))

#def <- defData(varname = "male", formula = .4, dist = "binary")

dt[, .N, keyby = Graft] |> gt::gt()
...
dt1 <- addCondition(defC, dt, newvar = "outcome")

# Verify the rates
dt1[, mean(outcome), by = Graft] |> gt::gt() |> fmt_number(columns = V1, decimals = 2)
...
grim_probability(x = "0.47", n = 53, percent = TRUE)
...
This one sits just on the margin of acceptability (\~0.80), but I can already hear the angry comments getting typed up to reply "it's close enough, back off". I think its a margin call, but either way it needs to be brought into the discussion about this trial.
...
I wrote a (fairly lengthy) function to simulate the dataset and re-plot the survival curves using *survival* (v`{r} utils::packageVersion("survival")`) [@survival] and *ggplot2* (v`{r} utils::packageVersion("ggplot2")`) [@ggplot2] packages.
...
Well, first of all - I'm not sure this would pass HREC in this day and age. ACL reconstruction in skeletally immature individuals is a different set of ethical considerations to adults and I'm not sure that would pass muster in the present environment. Considering the narrowness of the inclusion critera, adding in immature patients into the sample is an interesting choice. The inclusion of paediatric patients also raises issues for the validity of the PROMs included in the outcomes (see CONSORT 6).
