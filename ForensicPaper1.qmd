...
You can review the abstract for this paper [here](https://journals.sagepub.com/doi/10.1177/0363546506296042). In brief, this was an attempted RCT comparing two ways of reconstructing the anterior cruciate ligament in patients participating in high levels of competitive sport at the time of injury. The target paper is the 10-year follow up of the comparison between groups, with previous reports at 2, 5 and 7 year follow ups (see Trial Protocol).
...
mem_stringdist <- memoise::memoise(stringdist)
mem_str_detect <- memoise::memoise(str_detect)
...
author_full_names <- TargetCitation$data$author[[1]] |>
  dplyr::mutate(full_name = paste(given, family)) %>%
  dplyr::pull(full_name)
```

```{r}

# For detailed progress messages
...
Primary and secondary outcomes were not specified in any of the trial reports.
...
Determination of the required sample size has not been described in this paper or any others in the series. I will focus this report on a quick assessment of the estimated power

I'm going to start with the *IDEAL* *outcome* (see CONSORT 6a) and assess the achieved power. This is where I came up against the first consistency issue in this paper. using the *simstudy* package (v`{r} utils::packageVersion("simstudy")`) [@simstudy] to recreate the dataset.

So I created the datatable of groupings directly using the numbers described in Table 5 of the paper. This *should* be the floor sample sizes available, given the availability of assessment data for both groups. Ran a quick check to make sure the numbers came across ok.
...
IdealSim %>%
  group_by(Graft) %>%
  summarize(proportion = mean(Outcome))

```

So this led to the *scrutiny* package (v`{r} utils::packageVersion("scrutiny")`) [@scrutiny] to run a *GRIM* test on the proportions.
...
grim_probability(x = "69", n = 58, percent = TRUE)
...
I wrote a (fairly lengthy) function to simulate the dataset and re-plot the survival curves using *survival* (v`{r} utils::packageVersion("survival")`) [@survival] and *ggsurvfit* (v`{r} utils::packageVersion("ggsurvfit")`) [@ggplot2] packages.
...
Well, first of all - I'm not sure this would pass HREC in the present day. ACL reconstruction in skeletally immature individuals is a different set of ethical considerations to adults and I'm not sure that would pass muster in the present environment. Considering the narrowness of the inclusion criteria, adding in immature patients into the sample is an interesting choice. The inclusion of paediatric patients also raises issues for the validity of the PROMs included in the outcomes (see CONSORT 6).


I wanted to know if the proportion of females is generalisable, so I established the female proportion in both study groups combined, compared with the New Zealand ACL Registry data in the latest report [@aclregistry2024].

```{r}
SexCheck <- list(
  Group1 = prop.test(Group1$FemaleCount + Group2$FemaleCount, 180, 0.95),
...
```{epoxy}
A quick GRIM check of the female proportion returns [{scrutiny::grim(as.character(SexCheck$Group1$estimate), n = 90, percent = TRUE)}] 
```


```{epoxy}

The proportion of females in the selected study {round(SexCheck$Group1$estimate,2)} ({round(SexCheck$Group$conf.int[1],digits = 2)}, {round(SexCheck$Group1$conf.int[2],digits = 2)}) is in line with the NZ ACL Registry {round(SexCheck$NZRegistry$estimate,digits = 2)} ({round(SexCheck$NZRegistry$conf.int[1],digits = 2)}, {round(SexCheck$NZRegistry$conf.int[2],digits = 2)})

```

```{epoxy}

Checks for each Group for Age came out [{scrutiny::grim(as.character(Group1$AgeMedian), n = 90)}] and [{scrutiny::grim(as.character(Group2$AgeMedian), n = 90)}].
...
This is a bit of a mystery. Without access to a protocol prepared prior to the first follow up report, its difficult to know what has been omitted.
